...
function OverviewPageContent() {
  ...
  useEffect(() => {
    ...
    // Repair lineups generated by the previous keeper-first ordering. This is
    // deliberately narrow: a user lineup is rebuilt only when the squad owns
    // a designated partnership but those two players are not the two openers.
    const specialPair = findSpecialOpenerPair(userSquadPlayers);
    const hasSpecialPairOpening = (lineup: readonly string[]) => Boolean(
      specialPair
      && lineup.length === 11
      && specialPair.every((player) => lineup.slice(0, 2).includes(player.id)),
    );
    const battingPairNeedsRepair = Boolean(
      specialPair && battingFirstXI.length === 11 && !hasSpecialPairOpening(battingFirstXI),
    );
    const bowlingPairNeedsRepair = Boolean(
      specialPair && bowlingFirstXI.length === 11 && !hasSpecialPairOpening(bowlingFirstXI),
    );

    if (battingPairNeedsRepair || bowlingPairNeedsRepair) {
      const repairedPlans = buildAiMatchLineups(userSquadPlayers, {
      ...
      const repairedBattingXI = battingPairNeedsRepair
        ? repairedPlans.battingFirst.startingXI
        : battingFirstXI;
      const repairedBowlingXI = bowlingPairNeedsRepair
        ? repairedPlans.bowlingFirst.startingXI
        : bowlingFirstXI;
      const repairedBattingImpactSubs = battingPairNeedsRepair
        ? buildRecommendedImpactSubs(repairedBattingXI, userSquadCandidates, "battingFirst")
        : battingFirstImpactSubs;
      const repairedBowlingImpactSubs = bowlingPairNeedsRepair
        ? buildRecommendedImpactSubs(repairedBowlingXI, userSquadCandidates, "bowlingFirst")
        : bowlingFirstImpactSubs;
      const resetBattingImpact = Boolean(
        battingFirstImpactPlayerId && repairedBattingXI.includes(battingFirstImpactPlayerId),
      );
      const resetBowlingImpact = Boolean(
        bowlingFirstImpactPlayerId && repairedBowlingXI.includes(bowlingFirstImpactPlayerId),
      );

      setBattingFirstXI(repairedBattingXI);
      setBowlingFirstXI(repairedBowlingXI);
      setBattingFirstImpactSubs(repairedBattingImpactSubs);
      setBowlingFirstImpactSubs(repairedBowlingImpactSubs);
      if (resetBattingImpact) {
        setBattingFirstImpactPlayerId(null);
        setBattingFirstOutgoingPlayerId(null);
      }
      if (resetBowlingImpact) {
        setBowlingFirstImpactPlayerId(null);
        setBowlingFirstOutgoingPlayerId(null);
        setBowlingFirstImpactBattingPosition(null);
      }
      saveCareerState({
        battingFirstXI: repairedBattingXI,
        bowlingFirstXI: repairedBowlingXI,
        battingFirstImpactSubs: repairedBattingImpactSubs,
        bowlingFirstImpactSubs: repairedBowlingImpactSubs,
        ...(resetBattingImpact
          ? { battingFirstImpactPlayerId: null, battingFirstOutgoingPlayerId: null }
          : {}),
        ...(resetBowlingImpact
          ? {
