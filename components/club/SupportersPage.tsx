          ...
          <div className="grid h-full min-h-0 grid-cols-[.9fr_1.1fr] gap-3">
            ...
            <section className="min-h-0 overflow-hidden border border-border bg-surface2 p-3">
              ...
              <div className="grid h-[calc(100%-26px)] min-h-0 grid-cols-2 grid-rows-5 gap-2">
                {view.eventLedger.slice(0, 10).map((event) => <article key={event.id} className="min-h-0 overflow-hidden border border-border bg-surface px-2 py-1.5"><div className="flex items-center gap-2"><span className={`font-anton text-sm ${event.impact >= 0 ? "text-emerald-500" : "text-rose-500"}`}>{event.impact > 0 ? `+${event.impact}` : event.impact}</span><h4 className="truncate text-[8px] font-bold text-text-primary">{event.title}</h4></div><p className="truncate text-[7px] text-text-secondary">{formatDisplayDate(event.date)} · {event.category} · {event.detail}</p></article>)}
              </div>
            </section>
          </div>
        )}
