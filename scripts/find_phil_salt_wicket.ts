...
async function main() {
  ...
  for (let sIndex = 1; sIndex <= 50; sIndex++) {
    resetLotCache();
    resetAuctionQuirks();
    ...
    for (const m of fixtures) {
      ...
      const res = simulateInstantMatch({ fixtureId: m.id, matchNumber: m.matchNumber, seed: `sim-50-${sIndex}-${m.matchNumber}`, teamA, teamB, players: playerMap, teamAPlans: buildPlansForTeam(teamA, cond), teamBPlans: buildPlansForTeam(teamB, cond), conditions: cond, time: m.matchNumber % 2 === 0 ? "19:30" : "15:30", stage: m.stage } as any);

      res.innings.forEach((inn, innIdx) => {
        inn.bowling.forEach((bw) => {
          if ((bw.name?.includes("Salt") || bw.id?.includes("salt")) && bw.wickets > 0) {
            console.log(`\n🎉 FOUND PHIL SALT WICKET!`);
            console.log(`Season: ${sIndex} | Match: ${m.matchNumber} (${teamA.shortName} vs ${teamB.shortName})`);
            inn.oversDetail.forEach((ov) => {
              ov.deliveries.forEach((d) => {
                if (d.bowlerId === bw.id && d.wicket) {
                  console.log(`Dismissed Batter: ${d.wicket.dismissedBatterName || d.wicket.dismissedBatterId} (${d.wicket.kind})`);
                }
              });
            });
          }
