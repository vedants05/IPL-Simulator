...
async function main() {
  ...
  let mainSpecialistOvers = 0;
  let partTimerOvers = 0;
  let nonBowlerOvers = 0;
  let deathOversTotal = 0;
  let deathSpecialistOvers = 0;
  ...
  for (let sIndex = 1; sIndex <= TOTAL_SEASONS; sIndex++) {
    ...
    for (const m of fixtures) {
      ...
      const res = simulateInstantMatch({ fixtureId: m.id, matchNumber: m.matchNumber, seed: `sim-rot-${sIndex}-${m.matchNumber}`, teamA, teamB, players: playerMap, teamAPlans: buildPlansForTeam(teamA, cond), teamBPlans: buildPlansForTeam(teamB, cond), conditions: cond, time: m.matchNumber % 2 === 0 ? "19:30" : "15:30", stage: m.stage } as any);
      ...
      res.innings.forEach((inn) => {
        inn.bowling.forEach((bw) => {
          ...
          if (!p || p.currentBowling < 45 || p.role === "WK-Batsman") {
            nonBowlerOvers += overs;
          } else if (p.currentBowling < 60) {
            partTimerOvers += overs;
          } else {
            mainSpecialistOvers += overs;
          }
        });

        inn.oversDetail.forEach((ov) => {
          if (ov.overNumber >= 16) {
            deathOversTotal++;
            const bw = playerMap[ov.bowlerId];
            if (bw && bw.currentBowling >= 65 && bw.role !== "WK-Batsman") {
              deathSpecialistOvers++;
            }
          }
        });
      });
    }
    console.log(`✓ Season ${sIndex}/10 completed...`);
  }

  console.log("\n==================================================================================");
  console.log("   BOWLING ROTATION VERIFICATION REPORT (10 SEASONS / 740 MATCHES)                 ");
  console.log("==================================================================================");
  console.log(` • Total Overs Bowled: ${totalOversBowled} overs`);
  console.log(` • Main Specialist Overs (Rating >= 60): ${mainSpecialistOvers} overs (${((mainSpecialistOvers / totalOversBowled) * 100).toFixed(2)}%)`);
  console.log(` • Part-Timer Overs (Rating 45-59): ${partTimerOvers} overs (${((partTimerOvers / totalOversBowled) * 100).toFixed(2)}%)`);
  console.log(` • Non-Bowler Overs (Rating < 45 / WK): ${nonBowlerOvers} overs (${((nonBowlerOvers / totalOversBowled) * 100).toFixed(2)}%)`);
  console.log(` • Death Overs (Overs 16-20) Specialist %: ${((deathSpecialistOvers / deathOversTotal) * 100).toFixed(2)}% (${deathSpecialistOvers} / ${deathOversTotal} overs)`);
  console.log("==================================================================================");
}
